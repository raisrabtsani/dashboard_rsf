...
function chartSegmenData(segmen) {
    const sumber = grafikSegmen[tabTrendSegmen.value];
    const seri = (sumber?.seri ?? []).find(
        (item) => String(item.segmen).toLowerCase() === String(segmen).toLowerCase(),
    );

    if (!seri) return null;

    const labels = (sumber?.tanggal ?? []).map((tanggal) => String(Number(String(tanggal).slice(-2))));

    return {
        labels,
        datasets: [
            {
                label: tabTrendSegmen.value === 'total' ? 'Total' : tabTrendSegmen.value.toUpperCase(),
                borderColor: WARNA_TREND[tabTrendSegmen.value],
                backgroundColor: WARNA_TREND[tabTrendSegmen.value],
                data: seri.titik.map((titik) => titik.nilai ?? null),
                spanGaps: false,
            },
        ],
    };
...
const cakupanData = computed(() => {
    if (applied.uker_id) return namaPilihan(opsi.uker, applied.uker_id, 'Unit Kerja terpilih');
    if (applied.cabang_id) return namaPilihan(opsi.cabang, applied.cabang_id, 'Cabang terpilih');
    if (applied.area_id) return namaPilihan(opsi.area, applied.area_id, 'Area terpilih');
            ...
            <section class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-200/80">
                ...
                <div class="grid grid-cols-2 gap-2.5 p-3 sm:grid-cols-3 xl:grid-cols-[1.05fr_1fr_1fr_1fr_1fr_1fr_auto_auto]">
                    <label class="block min-w-0">
                        ...
                        <input v-model="pending.tanggal" type="date" class="filter-control" />
                    </label>
                    ...
                    <label class="block min-w-0">
                        ...
                        <select class="filter-control text-slate-500" disabled>
                    ...
                    <label class="block min-w-0">
                        ...
                        <select class="filter-control text-slate-500" disabled>
                            <option>Semua Segmentasi</option>
                        </select>
                    </label>
                    <button
                        type="button"
            ...
            <section v-if="scope.bolehLihatRanking.value" class="dashboard-card relative overflow-hidden">
                ...
                <div class="flex flex-wrap items-end justify-between gap-3 border-b border-slate-100 px-4 py-3">
                    ...
                    <div class="flex flex-wrap items-end gap-2">
                        <label class="block">
                            <span class="mini-filter-label">Area / Uker</span>
                            <select v-model="drilldown" class="mini-filter-control">
                        ...
                        <label class="block">
                            ...
                            <select class="mini-filter-control text-slate-500" disabled>
                                <option>Semua Segmentasi</option>
                            </select>
                        </label>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="(b, i) in barisTerurut" :key="b.id" class="transition hover:bg-blue-50/30">
                                ...
                                <td class="px-3 py-2">
                                    ...
                                    <p class="mt-0.5 text-[8px] font-medium text-blue-500">Area Jakarta</p>
                                </td>
