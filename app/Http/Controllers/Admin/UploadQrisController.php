...
use App\Services\QrisCsvImportService;
...
class UploadQrisController extends Controller
{
    public function __construct(private readonly QrisCsvImportService $service) {}

    public function index(): Response
    {
        return Inertia::render('Admin/Upload/Qris', [
            'kolomBerkas' => QrisCsvImportService::KOLOM,
        ]);
    }
    ...
    public function unduh(string $tanggal): StreamedResponse
    {
        ...
        abort_if($baris === [], 404, "Tidak ada data QRIS untuk tanggal {$tanggal}.");

        return response()->streamDownload(function () use ($baris) {
            $keluaran = fopen('php://output', 'w');
            fputcsv($keluaran, QrisCsvImportService::KOLOM, escape: '');
            ...
            fclose($keluaran);
        }, "qris-{$tanggal}.csv", ['Content-Type' => 'text/csv']);
    }
