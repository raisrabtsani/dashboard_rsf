...
use App\Services\SimpananWholesaleCsvImportService;
...
/**
 * Controller TIPIS — parsing di SimpananWholesaleCsvImportService.
 */
class UploadSimpananWholesaleController extends Controller
{
    public function __construct(private readonly SimpananWholesaleCsvImportService $service) {}

    public function index(): Response
    {
        return Inertia::render('Admin/Upload/SimpananWholesale', [
            'kolomBerkas' => SimpananWholesaleCsvImportService::KOLOM,
        ]);
    }
    ...
    public function unduh(string $tanggal): StreamedResponse
    {
        ...
        abort_if($baris === [], 404, "Tidak ada data DPK Wholesale untuk tanggal {$tanggal}.");

        return response()->streamDownload(function () use ($baris) {
            $keluaran = fopen('php://output', 'w');
            fputcsv($keluaran, SimpananWholesaleCsvImportService::KOLOM, escape: '');
            ...
            fclose($keluaran);
        }, "dpk-wholesale-{$tanggal}.csv", ['Content-Type' => 'text/csv']);
    }
